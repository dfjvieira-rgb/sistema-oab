...
export const FolhaEngine = {
    montar: (containerId) => {
        ...
        editor.id = 'editor-principal'; // ID CRÍTICO PARA O FIREBASE
    ...
    // Injeção de textos (útil para o botão Pergaminho/Estruturas)
    ...
    // Renderização para Impressão/PDF conforme solicitado
    renderizarFolhaEstatica: (dadosBrutos, missão) => {
        ...
        let html = `
            <div style="text-align:center; border-bottom:3px solid #000; padding:20px; font-family:Arial,sans-serif; background:#f8fafc; color:black;">
                <h2 style="margin:0; font-size:18px;">CADERNO DE RESPOSTAS PM 2026 - MISSÃO: ${missão}</h2>
                <p style="margin:5px 0 0 0; font-size:12px; color:#444;">SISTEMA OPERACIONAL QG JOAQUIM</p>
            </div>
        `;
